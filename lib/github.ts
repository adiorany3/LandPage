...
const knownDescriptions: Record<string, string> = {
  ...
  deteksiayam: "Aplikasi untuk membantu mendeteksi penyakit pada unggas melalui citra kotoran. Hasil bukan diagnosis dokter hewan.",
...
function toProject(repository: GitHubRepositoryResponse): GitHubProject {
  ...
  return {
    ...
    description: repository.name === "deteksiayam" ? knownDescriptions.deteksiayam : repository.description?.trim() || fallbackDescription(repository.name, category),
    url: repository.html_url,
    homepage: repository.name === "deteksiayam" ? "https://deteksiayam.streamlit.app/" : repository.homepage?.trim() || null,
...
function fallbackProjects(): GitHubProject[] {
  return fallbackRows.map((item) => {
    ...
    return {
      ...
      homepage: item.name === "deteksiayam" ? "https://deteksiayam.streamlit.app/" : null,
